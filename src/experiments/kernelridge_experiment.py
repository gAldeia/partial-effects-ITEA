...
filelock_name     = './locks/kernelridge_lock.txt.lock'
fname_globalexpl    = f'../../results/tabular_raw/kernelridge-globalexplanations.csv'
fname_localexpl     = f'../../results/tabular_raw/kernelridge-localexplanations.csv'
fname_regressionres = f'../../results/tabular_raw/kernelridge-regressionresults.csv'

n_runs          = 30

configurations = {
    'alpha'  : [2, 3, 5],
    'kernel' : ['chi2', 'linear', 'poly', 'rbf'],
    'gamma'  : [1e-5, 1e-3, 1e-1],
    'degree' : [5]
}

explainers          = ['SHAP', 'SHAP_adj', 'LIME']
# ------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    for ds in sys.argv[1:]:
        ...
        columns = [ # All column names should be capitalized
            ...
            'RMSE-train-KR',   # Final RMSE error on training data
            'RMSE-test-KR',    # Final RMSE error on training data
            'Tottime-train-KR' # Training time
        ] + [
            f'Tottime-{explainer}-KR' 
        ...
        for rep in range(n_runs):
            ...
            TimeTrain = time.time()
            kr = KernelRidge(**configuration).fit(Xtrain, ytrain)
            ...
            for explainer in explainers:
                ...
                save_explanations(
                    ...
                    dataset       = ds,
                    rep           = rep,
                    explainer     = f'Global-{explainer}-KR',
                ...
                save_explanations(
                    ...
                    dataset       = ds,
                    rep           = rep,
                    explainer     = f'Local-{explainer}-KR',
            ...
            with FileLock(filelock_name):
                ...
                results['RMSE-train-KR'   ].append(mean_squared_error(kr.predict(Xtrain), ytrain, squared=False))
                results['RMSE-test-KR'    ].append(mean_squared_error(kr.predict(Xtest), ytest, squared=False))
                results['Tottime-train-KR'].append(TimeTrain)

                for i, explainer in enumerate(explainers):
                    results[f'Tottime-{explainer}-KR'].append(tottimes[i])
