...
filelock_name     = './locks/randomforest_lock.txt.lock'
fname_globalexpl    = f'../../results/tabular_raw/randomforest-globalexplanations.csv'
fname_localexpl     = f'../../results/tabular_raw/randomforest-localexplanations.csv'
fname_regressionres = f'../../results/tabular_raw/randomforest-regressionresults.csv'
...
if __name__ == '__main__':
    ...
    for ds in sys.argv[1:]:
        ...
        columns = [ # All column names should be capitalized
            ...
            'RMSE-train-RF',   # Final RMSE error on training data
            'RMSE-test-RF',    # Final RMSE error on training data
            'Tottime-train-RF' # Training time
        ] + [
            f'Tottime-{explainer}-RF' 
        ...
        for rep in range(n_runs):
            ...
            TimeTrain = time.time()
            rf = RandomForestRegressor(**configuration).fit(Xtrain, ytrain)
            ...
            for explainer in explainers:
                ...
                save_explanations(
                    ...
                    dataset       = ds,
                    rep           = rep,
                    explainer     = f'Global-{explainer}-RF',
                ...
                save_explanations(
                    ...
                    dataset       = ds,
                    rep           = rep,
                    explainer     = f'Local-{explainer}-RF',
            ...
            with FileLock(filelock_name):
                ...
                results['RMSE-train-RF'   ].append(mean_squared_error(rf.predict(Xtrain), ytrain, squared=False))
                results['RMSE-test-RF'    ].append(mean_squared_error(rf.predict(Xtest), ytest, squared=False))                
                results['Tottime-train-RF'].append(TimeTrain)

                for i, explainer in enumerate(explainers):
                    results[f'Tottime-{explainer}-RF'].append(tottimes[i])
